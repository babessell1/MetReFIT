function [addRxns,blockedMets,viableMets] = metREFit_all(model,modelT,presentDB,superDB,orthoDB,transDB,blockedMets,viableMets,demandRxns,masterMetsDB)
...
while ~escapeFlag
    ...
    for m = 1:length(checkMets)
        ...
        if ~solvedFlag
            [testRxns_uni] = addFromUniversal(met, traceNum, viableMets, excludeRxns, masterDB, presentDB);
        end
        
        if ~solvedFlag && ~isempty(testRxns_uni)
                [solvedFlag, model, modelT, blockedMets, viableMets, solvedRxns]...
                    = testSolution_type3(model, modelT, blockedMets,...
                        checkMets, viableMets, testRxns_uni, demandRxns);
        end
