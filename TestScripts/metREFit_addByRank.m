function [solvedRxns,blockedMets,viableMets] = metREFit_addByRank(model, modelT, presentDB,superDB,orthoDB,blockedMets,viableMets,demandRxns,masterMetsDB)
...
% %         for r = 1:length(testRxns_temp(:,1))
% %             rxnChain = {};
% %             rxnChain = vertcat(rxnChain,testRxns_temp(r,:)); 
% %             cscore = cell2mat(testRxns_temp(r,9));
...
% %             if missReacts == 1
% %                 traceNum = traceNum + 1;
% %                 chainMet = testRxns_temp{r,17};
% %                 chainRxns_temp = {};
% %                 if length(chainMet) == 1
% %                     [chainRxns_temp] = addFromSuper(chainMet, traceNum, viableMets,...
% %                                     excludeRxns,superDB,orthoDB,presentDB);
% %                 end
% % 
% %                 if ~isempty(chainRxns_temp)
% %                     for s = 1:length(chainRxns_temp(:,1))
% %                         cscore = cell2mat(chainRxns_temp(s,9));
% %                         missReacts = cell2mat(chainRxns_temp(s,10));
% %                         if missReacts == 0
% %                             rxnChain_temp = vertcat(rxnChain,chainRxns_temp(s,:));
% %                             [solvedFlag, model, modelT, blockedMets, viableMets, solvedRxns]...
% %                                 = testSolution_type4(model, modelT, blockedMets, checkMets,...
% %                                      viableMets, rxnChain_temp, demandRxns);
% %                         end
% %                         if solvedFlag 
% %                              chainSolve = 1;
% %                              break;
...
% %                 end
% %                 if chainSolve
% %                     addRxns = vertcat(addRxns,rxnChain_temp);
% %                     
% %                     break;
% %                 end
% %             end
...
% %             addRxns = vertcat(addRxns, rxnChain_temp);
% %             save(savedWorkspace, 'model', 'modelT', 'addRxns');
% %             numSol = length(rxnChain_temp(:,1));
% %             fprintf("%Found rank validated solution for %s\n\n", numSol, met);
% %             disp(rxnChain_temp(:,1));
% % 
% %         else
% %             fprintf("No solutions for %s could be validated\n\n", met);
% %         end
